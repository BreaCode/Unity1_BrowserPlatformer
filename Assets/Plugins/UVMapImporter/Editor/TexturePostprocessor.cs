...
namespace Editor
{
    ...
    {
        private const string EXTENTION_PNG = ".png";
        private const string PIXEL_MAP_SUFFIX = ".map";
        private const string PIXEL_WEIGHTS_PREFIX = "source.";
        ...
        private void OnPostprocessTexture(Texture2D texture)
        {
            ...
            if (extention != EXTENTION_PNG)
            {
                Debug.Log("Wrong extention: " + extention);
                return;
            }
            ...
            else if (fileName.StartsWith(PIXEL_WEIGHTS_PREFIX))
            {
                ProcessPixelWeights(texture, fileName.Replace(PIXEL_WEIGHTS_PREFIX, ""));
            }
        }
        ...
        private void ProcessPixelWeights(Texture2D texture, string fileName)
        {
            ...
            var mapName = fileName.Split('_')[0] + PIXEL_MAP_SUFFIX;
            var map = FindPixelMap(mapName);
            if (map == null)
            {
                Debug.Log("Wrong map name:" + mapName);
                return;
            }
            ...
            var path = assetPath.Replace(PIXEL_WEIGHTS_PREFIX, "");
